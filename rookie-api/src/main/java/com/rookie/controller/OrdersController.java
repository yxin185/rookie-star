...
//        System.out.println(submitOrderBO.toString());
        ...
        String shopcartJson = redisOperator.get(ROOKIE_SHOPCART + ":" + submitOrderBO.getUserId());
        if (StringUtils.isBlank(shopcartJson)) {
            return RookieJsonResult.errorMsg("购物车数据不正确");
        }
        ...
        redisOperator.set(ROOKIE_SHOPCART + ":" + submitOrderBO.getUserId(), JsonUtils.objectToJson(shopcartBOList));

        // TODO 整合redis之后，完善购物车中的已结算商品清除，并且同步到前端的cookie
        CookieUtils.setCookie(request, response, ROOKIE_SHOPCART, JsonUtils.objectToJson(shopcartBOList), true);
